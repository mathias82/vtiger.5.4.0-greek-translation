<?php
/*********************************************************************************
** The contents of this file are subject to the vtiger CRM Public License Version 1.0
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is:  vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
* 
 ********************************************************************************/
/*********************************************************************************
GREEK TRANSLATION - LOCALISATION

Prepared on February, 2013 (for vTigerCrm 5.4.0)

Contributors:
mathias stavrou m_stauroy@hotmail.com, Open For Business, http://www.mstavrou.com for HELLASCRM http://www.hellascrm.gr , Athens, Greece
 ********************************************************************************/
 
$mod_strings = Array (
'LBL_CRM_SETTINGS' => 'Ρυθμίσεις',
'Tooltip' => 'ToolTip',
'LBL_TOOLTIP_MANAGEMENT'=>'Διαχείριση Tooltip',
'LBL_TOOLTIP_MANAGEMENT_DESCRIPTION'=>'Διαχειριστείτε τις πληροφορίες των tooltip από εδώ',
'LBL_FIELDS_IN'=>'Πεδία σε',
'LBL_TOOLTIP_HELP_TEXT'=>'Επιλέξτε τα πεδία που θέλετε νε εμφανίζονται σαν tooltip',
'LBL_FIELD'=>'Πεδίο',

);

?>
