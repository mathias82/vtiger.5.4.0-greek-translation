<?php
/*+**********************************************************************************
 * The contents of this file are subject to the vtiger CRM Public License Version 1.0
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is:  vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
 ************************************************************************************/
/*********************************************************************************
GREEK TRANSLATION - LOCALISATION

Prepared on February, 2013 (for vTigerCrm 5.4.0)

Contributors:
mathias stavrou m_stauroy@hotmail.com, Open For Business, http://www.mstavrou.com for HELLASCRM http://www.hellascrm.gr , Athens, Greece
 ********************************************************************************/
 
 
$mod_strings = Array(
/*some general information*/
'LBL_MODULE_NAME'=>'Πάγια',
'SINGLE_Assets'=>'Πάγια',

/*blocks for the module*/
'LBL_ASSET_INFORMATION'=>'Πληροφορίες για Περιουσία',
'LBL_CUSTOM_INFORMATION'=>'Πληροφορίες Πελάτη',
'LBL_DESCRIPTION_INFORMATION'=>'Σημειώσεις',
'LBL_CRM_SETTINGS' => 'Ρυθμίσεις',

/*fields for the module*/
'Assets'=>'Πάγια Στοιχεία',
'Asset Name' => 'Όνομα',
'Customer Name'=>'Όνομα Πελάτη',
'Product Name'=>'Όνομα Προιόντος',
'Serial Number'=>'Σειριακός Αριθμός',
'Asset No'=>'Αριθμός στοιχείου',
'Date Sold'=>'Ημερομηνία Πώλησης',
'Date in Service'=>'Ημερομηνία για ',
'Status'=>'Status',
'Shipping Method'=>'Μέθοδος Αποστολής',
'Shipping Tracking Number'=>'Αριθμός αναζήτησης αποστολής',
'Tag Number'=>'Αριθμός ετικέτας',
'Notes'=>'Σημειώσεις',
'Invoice Name'=>'Τιμολόγιο Όνομα',

/*picklist values*/
'In Service'=>'Στην Υπηρεσία',
'Out-of-service'=>'Εκτός Υπηρεσίας',


);
?>
